...
struct TimeSuggestion: Identifiable {
    var id: String { label + (isAIParsed ? "-ai" : "") }
    let label: String
    let date: Date
    let formattedDate: String
    var isAIParsed: Bool = false
}
...
class NaturalLanguageDateParser {
    ...
    func suggestions(for input: String) -> [TimeSuggestion] {
    ...
    func defaultSuggestions() -> [TimeSuggestion] {
        let now = Date()
        ...
        return [
            ...
            TimeSuggestion(label: "Someday", date: somedayDate, formattedDate: "¯\\_(ツ)_/¯"),
        ]
    }
        ...
        for kw in keywords {
            if kw.match.hasPrefix(input) || input.hasPrefix(kw.match) {
                if seenLabels.insert(kw.label).inserted {
                    let date = kw.date()
                    results.append(TimeSuggestion(label: kw.label, date: date, formattedDate: formatDate(date)))
                }
            }
        }
    ...
    // Overload accepting Weekday enum
    ...
    private func generateSomedayDate() -> Date {
        let calendar = Calendar.current
        let now = Date()
        let randomDays = Int.random(in: 1...7)
    ...
    func aiSuggestion(for input: String) async -> TimeSuggestion? {
